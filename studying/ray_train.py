...
def create_my_trainer(rl_algorithm: str):
    ...
    class MyTrainer(RLTrainer):
        def setup(self, config):
            ...
            self.trainer_config = config.pop("tariner_config")
...
if __name__ == "__main__":
    ...
    # Path: change string to object
    # resolve: change to absolute path
    ...
    config["rollout_fragment_length"] = config["train_bath_size"] / (config["num_workers"] * config["num_envs_per_worker"])

    local_dir = "./ray_results"
    algorithm = config["trainer_config"]["algorithm"] # PPO
