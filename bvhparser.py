...
class MyBVH():
    def __init__(self, filename, bvh_info, skel, T_frame = None):
        ...
        ## convert mocap data to transformation matrix
        self.mocap_refs_mat = {}
        self.Tpose_mat = {}
        ...
        for jn in self.mocap.get_joints():
            ...
            if len(joint_chs) == 6:
                ...
                for i in range(self.mocap.nframes):
                    T_mat = np.identity(4); T_mat[:3, :3] = eulerToMatrix(rot[i], euler_order); T_mat[0:3, 3] = pos[i]  
    ...
    def setRefs(self):
        ...
        self.root_T = self.root_jn.convertToTransform(self.mocap_refs[-1,  0:self.root_jn.getNumDofs()]).matrix() # @ np.linalg.inv(root_0)
        self.root_T[:3,:3] = inplaneXY(self.root_T)

        self.root_T = self.root_T @ np.linalg.inv(root_0)
        self.root_T[1,3] = 0.0

        # self.root_T[:3, 3] = self.root_T[:3,:3] @ self.root_T[:3, 3] 
